...
class ConvNet(hk.Module):
    ...
    def __init__(self, output_size, conv_layers=2, kernel_shape=3, channels=32, dense_size=256, bn_config=None,
                 logits_config=None, name=None):
        super().__init__(name=name)
        self.output_size = output_size
    ...
    def __call__(self, x, is_training, test_local_stats=False):
        ...
        for n in range(1, self.conv_layers):
            net = NormBlock(self.channels, stride=2, bn_config=self.bn_config, name="normblock_" + str(n) + "_x2")(net,
                                                                                                                   is_training)
            net = NormBlock(self.channels, stride=1, bn_config=self.bn_config, name="normblock_" + str(n) + "_x1")(net,
                                                                                                                   is_training)
...
class ResNet(hk.Module):
    ...
    def __init__(self, output_size, channels=64, bn_config=None, initial_conv_config=None, logits_config=None,
                 name=None):
        super().__init__(name=name)
        self.output_size = output_size
        self.channels = channels
