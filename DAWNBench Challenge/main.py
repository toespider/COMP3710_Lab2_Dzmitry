'''
Test CIFAR10 classifications with JAX and Haiku (DAWNBench Config)
'''
import haiku as hk
import jax
import numpy as np
...
#parameters
epochs = 24       # Reduced for super-convergence
batch_size = 512  # Increased to saturate A100 GPU
tfdtype = tf.float32
train_set = 'train'
download = True
...
# A wrapper function for updating seeds - Added Cutout!
def f(item):
    seed = rng_preproc.make_seeds(2)[0]
    return preprocess.preprocess_cifar(item, seed, cutout_size=8)
...
def _forward(batch: Batch, is_training: bool) -> jnp.ndarray:
    '''
    Forward pass through network using ResNet-18
    '''
    cnn = networks.ResNet(num_classes, channels=64)
    return cnn(batch["image"], is_training=is_training)

# Apply bfloat16 Mixed Precision Policy for A100 GPUs
policy = hk.mixed_precision.Policy(
    param_dtype=jnp.float32,
    compute_dtype=jnp.bfloat16,
    output_dtype=jnp.float32
)
hk.mixed_precision.set_policy(networks.ResNet, policy)
hk.mixed_precision.set_policy(networks.ResNetBlock, policy)
...
# Added nesterov=True for the super-convergence schedule
opt = optax.sgd(lr_schedule, momentum=0.9, nesterov=True)
...
for step in range(num_train_steps):
    if step % 100 == 0:
        ...
        print(f"[Step {step}/{num_train_steps}, Loss {loss:.5f}] Train / Test accuracy: "
